function updateForm() {
  ...
  function allIpsValid(ipAddrs) {
    ...
    validIpFilter = validIpArray.filter(function (e) {
      return e;
    });
  ...
  function ipValidList(ipAddrList) {
    ...
    for (let valIpNum = 0; valIpNum < validIpList.length; valIpNum++) {
      ...
      if (ipValidation(validIpRes)) {
        ...
        returnValidIp.push(
          "<font color='red'>" + validIpRes + " (INVALID)</font>"
        );
      }
    }
  ...
  function ciscoIpASA(ciscoIp) {
    var [ciscoOctet1, ciscoOctet2, ciscoOctet3, ciscoOctet4] =
      ciscoIp.split(".");
    return (
      decimalToHex(ciscoOctet1, 2) +
      decimalToHex(ciscoOctet2, 2) +
      decimalToHex(ciscoOctet3, 2) +
      decimalToHex(ciscoOctet4, 2)
    );
  }
  ...
  if (allIpsValid(ipValue)) {
    switch (wlcValue) {
      ...
      case "unifi":
        var opt43Result =
          vendorId + ipQuanValue + ciscoIpArray(removeTrailingComma(ipValue));
        break;
      case "vsz":
      case "zd":
        var opt43Result =
          vendorId +
          decimalToHex(ipLen) +
          convertToHex(removeTrailingComma(ipValue));
        break;
      default:
    ...
    switch (fwValue) {
      case "sonicwall":
        document.getElementById("Opt43String").innerHTML =
          parseForSonicwall(opt43Result);
        break;
      case "asa":
        document.getElementById("Opt43String").innerHTML = opt43Result;
        document.getElementById("cliString").innerHTML =
          "CLI Command: <b>option 43 hex " + opt43Result + "</b>";
        break;
      case "ndx":
      ...
      case "pfsense":
        document.getElementById("Opt43String").innerHTML =
          parseForpfSense(opt43Result);
        break;
      case "win":
        document.getElementById("Opt43String").innerHTML =
          parseForWindows(opt43Result);
        break;
      default:
    ...
    document.getElementById("Opt43String").innerHTML =
      "You entered an invalid IP address. Please check your entries and submit again.";
  }
